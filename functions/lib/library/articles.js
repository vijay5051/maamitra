...
// ── Hero image ──────────────────────────────────────────────────────────────
async function renderHeroImage(prompt, brand, preferred) {
    if (preferred === 'none')
        return { url: null, source: 'none', costInr: 0 };
    const styled = (0, brand_1.buildStyleLockedImagePrompt)(prompt, brand);
    let aiUrl = null;
    ...
    if (preferred === 'imagen') {
        ...
        aiUrl = await (0, imageSources_1.openaiImage)(styled, { quality: 'medium', size: '1536x1024' }).catch(() => null);
        if (aiUrl)
            costInr = 3.50;
    }
...
async function runArticleGenerator(input, actorEmail) {
    ...
    const topic = pickTopic(k.topics, typeof input.topic === 'string' ? input.topic : null, today, ageBucket);
