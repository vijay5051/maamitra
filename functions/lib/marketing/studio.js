...
function subjectObjectRules(userPrompt) {
    ...
    if (/\b(woollen|woolen|sweater|warm clothes|jacket)\b/.test(p))
        required.push('folded woollen sweater or warm clothes');
    if (/\bglove|gloves\b/.test(p))
        required.push('pair of gloves');
    if (/\bhat|cap|beanie\b/.test(p))
        required.push('woollen hat or beanie');
    if (/\bsnack|snacks|healthy snack|healthy snacks|food\b/.test(p))
        required.push('healthy snacks packed for the child');
...
function characterCountRules(userPrompt) {
    const p = userPrompt.toLowerCase();
    const roles = [];
    if (/\b(mother|mom|mumma|mama|maa)\b/.test(p))
        roles.push('one mother');
    if (/\b(kid|child|baby|toddler|son|daughter)\b/.test(p))
        roles.push('one child');
    if (/\b(grandma|grandmother|dadi|nani)\b/.test(p))
        roles.push('one grandmother');
    if (/\b(father|dad|papa|husband)\b/.test(p))
        roles.push('one father');
...
function buildGenerateStudioVariants(allowList) {
    return functions
        ...
        const model = data?.model === 'flux' ? 'flux' : 'dalle';
        ...
        for (let i = 0; i < variantCount; i++) {
            ...
            tasks.push((async () => {
                let imageUrl = null;
                let sourceUsed = model;
                try {
                    if (model === 'dalle') {
                        imageUrl = await (0, styleReferences_1.openaiMaaMitraReferenceImage)(styledPrompt, {
                            preset: 'post',
                            quality: 'medium',
                        ...
                        if (!imageUrl) {
                            imageUrl = await (0, imageSources_1.imagenGenerate)(styledPrompt, { aspectRatio: aspectRatio === '16:9' ? '16:9' : aspectRatio === '9:16' ? '9:16' : '1:1' });
                            if (imageUrl)
                                sourceUsed = 'imagen';
                        }
                    }
                ...
                try {
                    ...
                    await logCost({ source: sourceUsed, costInr: COST_PER_IMAGE[sourceUsed], bytes, actor: actor, meta: { variantId, mode } });
...
function tidyHashtags(brand) {
    const picked = brand.hashtags
        .filter((h) => typeof h === 'string' && h.trim().startsWith('#'))
        .slice(0, 5);
...
function humanizeScene(prompt) {
    const cleaned = prompt
        ...
        .replace(/[.。]+$/, '');
    if (!cleaned)
        return 'a small everyday parenting moment';
    return cleaned.charAt(0).toLowerCase() + cleaned.slice(1);
}
function fallbackStudioCaption(prompt, brand) {
