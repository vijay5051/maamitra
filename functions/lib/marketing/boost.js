...
exports.buildBoostMarketingDraft = buildBoostMarketingDraft;
...
function buildBoostMarketingDraft(allowList) {
    return functions
        ...
        if (dailyBudgetInr < 100)
            return { ok: false, code: 'budget-too-low', message: 'Minimum daily budget is ₹100.' };
        if (dailyBudgetInr > 5000)
            return { ok: false, code: 'budget-too-high', message: 'Max daily budget for now is ₹5000.' };
        if (durationDays < 1)
            return { ok: false, code: 'duration-invalid', message: 'Pick 1–7 days.' };
        ...
        try {
            ...
            const campaign = await postForm(`${GRAPH_BASE}/act_${encodeURIComponent(META_AD_ACCOUNT_ID)}/campaigns`, new URLSearchParams({
            ...
            const adSet = await postForm(`${GRAPH_BASE}/act_${encodeURIComponent(META_AD_ACCOUNT_ID)}/adsets`, new URLSearchParams({
            ...
            if (!adSetId)
                throw new Error('No ad set id returned');
            // 3. Creative — uses the existing IG post.
            const creative = await postForm(`${GRAPH_BASE}/act_${encodeURIComponent(META_AD_ACCOUNT_ID)}/adcreatives`, new URLSearchParams({
            ...
            if (!creativeId)
                throw new Error('No creative id returned');
            // 4. Ad
            await postForm(`${GRAPH_BASE}/act_${encodeURIComponent(META_AD_ACCOUNT_ID)}/ads`, new URLSearchParams({
