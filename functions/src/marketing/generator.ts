...
// Aggregate engagement rate per (pillar, template) over the last 30 days.
// pickSlot reads this and biases pillar selection toward winners; the
// caption prompt also gets the top 3 image prompts as inspiration.

interface PerfStats {
  byPillar: Map<string, { posts: number; avgRate: number }>;
  byPillarTemplate: Map<string, Map<string, { posts: number; avgRate: number }>>;
  topPrompts: string[];
}

async function loadPerformanceStats(): Promise<PerfStats> {
  const out: PerfStats = {
    byPillar: new Map(),
  ...
  try {
    ...
    const snap = await admin.firestore()
      ...
      .limit(200)
      .get();
    interface Row { pillar: string; template: string; rate: number; prompt: string }
    const rows: Row[] = [];
    for (const d of snap.docs) {
      const data = d.data() as any;
      if (data?.isSynthetic === true) continue;
      const m = data?.latestInsights;
      if (!m || typeof m?.reach !== 'number' || m.reach <= 0) continue;
...
function weightedPillarPick(pillars: Pillar[], stats: PerfStats): Pillar | null {
  ...
  if (totalPosts < 5) return pillars[Math.floor(Math.random() * pillars.length)];
  ...
  const weights = pillars.map((p) => {
    ...
    if (!stat || stat.posts < 2) return 1; // unseen / under-sampled → neutral
...
function pickSlot(brand: BrandKitData, override: GenerateInput, today: { weekdayKey: string; isoDate: string }, stats: PerfStats, promptOverride?: string | null): PickedSlot {
  ...
  if (!pillar) pillar = weightedPillarPick(brand.pillars, stats);
...
async function generateCaption(
  ...
  const template = forcedTemplate
    ? forcedTemplate
    : (['tipCard', 'quoteCard', 'milestoneCard', 'realStoryCard'] as TemplateName[]).includes(parsed?.template)
...
function templatePropsValid(template: TemplateName, props: Record<string, any> | null | undefined): boolean {
  if (!props || typeof props !== 'object') return false;
  const str = (v: unknown) => typeof v === 'string' && v.trim().length > 0;
  const arr = (v: unknown, min: number) =>
    Array.isArray(v) && v.length >= min && v.every((x: unknown) => str(x));
  switch (template) {
    case 'tipCard':
...
export async function runGenerator(
  ...
  const inputTemplate = ['tipCard', 'quoteCard', 'milestoneCard', 'realStoryCard'].includes(input.template as string)
    ? (input.template as TemplateName)
    : null;
  const forcedTemplate: TemplateName | null =
    inputTemplate ?? (slot.templateOverride && slot.templateOverride !== 'auto' ? slot.templateOverride : null);
  ...
  const requestedTemplate: TemplateName = forcedTemplate ?? captionOut.template;
  captionOut = {
    ...captionOut,
