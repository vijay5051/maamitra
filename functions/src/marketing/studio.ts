      ...
      for (let i = 0; i < variantCount; i++) {
        tasks.push((async () => {
          ...
          try {
            ...
            await logCost({ source: model, costInr: perImage, bytes, actor: actor as string | null, meta: { variantId } });
...
export function buildCreateStudioDraft(allowList: ReadonlySet<string>) {
  return functions
    ...
    .https.onCall(async (data: CreateDraftInput, context): Promise<CreateDraftResult> => {
      ...
      if (!imageUrl) return { ok: false, code: 'no-image', message: 'Pick an image first.' };
