...
async function fetchFbPostMetrics(fbPostId: string): Promise<PostInsightMetrics | null> {
  ...
  return functions
    ...
    .onRun(async () => {
      ...
      for (const docSnap of due.docs) {
        ...
        if (Object.keys(update).length > 0) await docSnap.ref.update(update);
      }
      return null;
    });
}
...
async function fetchFbAccountSnapshot(): Promise<FbAccountMetrics | null> {
  ...
  try {
    ...
    const pageRes = await fetch(
      `${GRAPH_BASE}/${META_FB_PAGE_ID}?fields=fan_count,followers_count&access_token=${encodeURIComponent(pat)}`,
    );
    if (!pageRes.ok) {
    ...
    const insRes = await fetch(
      `${GRAPH_BASE}/${META_FB_PAGE_ID}/insights?metric=page_impressions,page_impressions_unique&period=day&since=${yesterday}&until=${yesterday}&access_token=${encodeURIComponent(pat)}`,
    );
    let reach = 0;
    let impressions = 0;
    if (insRes.ok) {
...
export function buildPollMarketingAccountInsights() {
  return functions
    ...
    .onRun(async () => {
      ...
      const yesterdayData = (yesterdayDoc.exists ? yesterdayDoc.data() : {}) as any;
