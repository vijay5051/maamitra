...
type RetryArticleImageResult =
  | { ok: true; articleId: string; imageStatus: 'pending' | 'ready' | 'failed' }
  | { ok: false; code: string; message: string };
type SendArticleToMarketingDraftResult =
  | { ok: true; articleId: string; draftId: string }
  | { ok: false; code: string; message: string };

type ImageModel = 'openai' | 'lora' | 'imagen' | 'flux' | 'dalle' | 'pexels' | 'none';
...
interface ArticleMarketingSummary {
  headline: string;
  body: string;
  hashtags: string[];
}
...
function inferAgeBucketFromTopic(topic: string, buckets: AgeBucket[]): AgeBucket | null {
  const t = topic.toLowerCase();
  const wantedKey =
    /\b(pregnancy|pregnant|trimester|prenatal|antenatal)\b/.test(t) ? 'pregnancy' :
...
function fallbackArticleBody(topic: string, ageBucket: AgeBucket, brand: LibraryBrand, tone: string): ArticleDraftJson {
...
function weekdayKeyForNowIst(): 'mon' | 'tue' | 'wed' | 'thu' | 'fri' | 'sat' | 'sun' {
  const now = new Date(Date.now() + 5.5 * 3600 * 1000);
  return ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'][now.getUTCDay()] as 'mon' | 'tue' | 'wed' | 'thu' | 'fri' | 'sat' | 'sun';
}

function decodeStoragePathFromUrl(url: string | null | undefined): string | null {
...
async function loadMarketingHashtags(): Promise<string[]> {
  try {
    const snap = await admin.firestore().doc('marketing_brand/main').get();
    const raw = snap.exists ? (snap.data() as any)?.hashtags : null;
    if (!Array.isArray(raw)) return DEFAULT_MARKETING_HASHTAGS;
    const cleaned = raw
...
function buildLibraryArticleLink(articleId: string, article: any): string {
...
function assembleMarketingCaption(body: string, hashtags: string[], brandHashtags: string[], link?: string): string {
...
async function summarizeArticleForMarketing(
  article: any,
  brand: LibraryBrand,
): Promise<ArticleMarketingSummary> {
  ...
  const parsed = await chatJson<Partial<ArticleMarketingSummary>>([
    {
      role: 'system',
      content: [
        buildSystemVoiceHeader(brand),
...
function articleThumbnailAgeVisual(ageBucket?: AgeBucket, topic?: string): string {
...
function articleThumbnailGuard(ageBucket?: AgeBucket, topic?: string): string {
...
async function renderHeroImage(
  ...
  const styled = buildStyleLockedImagePrompt(`${prompt.trim()}\n${thumbnailGuard}`, brand);
...
function decideArticleStatus(
  explicit: 'published' | 'draft' | null,
  autoPublish: boolean,
  flags: { type: string; phrase: string }[],
): 'published' | 'draft' {
...
async function processArticleImageForDoc(
  ref: admin.firestore.DocumentReference,
  data: any,
): Promise<'ready' | 'failed'> {
  if (!data || data.source !== 'ai' || typeof data.aiImagePrompt !== 'string') return 'failed';

  const brand = await loadLibraryBrand();
  const preferred = (['openai', 'lora', 'imagen', 'dalle', 'flux', 'pexels', 'none'] as ImageModel[]).includes(data.aiRequestedImageModel)
    ? data.aiRequestedImageModel as ImageModel
    : 'openai';
  const ageBucket: AgeBucket | undefined =
    typeof data.aiAgeBucketKey === 'string' && typeof data.ageMin === 'number' && typeof data.ageMax === 'number'
...
async function createMarketingDraftFromArticle(
  ref: admin.firestore.DocumentReference,
  data: any,
  opts?: { actorEmail?: string | null; force?: boolean },
): Promise<{ ok: true; draftId: string } | { ok: false; code: string; message: string }> {
  if (!data || typeof data !== 'object') return { ok: false, code: 'invalid-data', message: 'Article data is missing.' };
  ...
  if (!title || !body) return { ok: false, code: 'missing-copy', message: 'Article title/body is incomplete.' };
  ...
  const compliance = runCompliance(`${social.headline}\n${social.body}`, brand);
...
export async function runArticleGenerator(
  ...
  const pickedAgeBucket = pickAgeBucket(
    k.ageBuckets,
    requestedAgeBucketKey,
    today,
  );
  ...
  const preferred: ImageModel = (['openai', 'lora', 'imagen', 'dalle', 'flux', 'pexels', 'none'] as ImageModel[]).includes(input.imageModel as any)
...
export function buildProcessPendingArticleImage() {
  return functions
...
export function buildRetryArticleImage(allowList: ReadonlySet<string>) {
  return functions
    .runWith({ memory: '1GB', timeoutSeconds: 300 })
    .https.onCall(async (data: { articleId?: unknown }, context): Promise<RetryArticleImageResult> => {
      ...
      if (!articleId) return { ok: false, code: 'missing-id', message: 'Provide an articleId.' };
      const ref = admin.firestore().collection('articles').doc(articleId);
      const snap = await ref.get();
      if (!snap.exists) return { ok: false, code: 'not-found', message: 'Article not found.' };
      const doc = snap.data() as any;
      if (doc?.source !== 'ai') return { ok: false, code: 'not-ai', message: 'Only AI articles support thumbnail retry.' };
...
export function buildSyncPublishedArticleToMarketingDraft() {
  return functions
...
export function buildSendArticleToMarketingDraft(allowList: ReadonlySet<string>) {
  return functions
    .runWith({ memory: '512MB', timeoutSeconds: 120 })
    .https.onCall(async (data: { articleId?: unknown }, context): Promise<SendArticleToMarketingDraftResult> => {
      if (!(await callerIsContentAdmin(context.auth?.token, allowList))) {
