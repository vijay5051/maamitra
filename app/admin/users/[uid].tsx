...
import { isAdminAiConfigured, summarizeUser } from '../../../services/adminAi';
  ...
  async function handleSummarize() {
    ...
    try {
      const daysSinceSignup = profile.createdAt
        ? Math.floor((Date.now() - new Date(typeof profile.createdAt === 'string' ? profile.createdAt : profile.createdAt?.toDate?.()).getTime()) / 86_400_000)
        : undefined;
      const text = await summarizeUser({
        name,
        email,
        state,
        stage,
        ...
        recentPostExcerpts: snap.recentPosts.slice(0, 3).map((p) => p.text?.slice(0, 140) ?? '').filter(Boolean),
        recentTicketSubjects: snap.recentTickets.slice(0, 3).map((t) => t.subject),
      });
      setAiSummary(text);
    } catch (e: any) {
        ...
        headerActions={
          snap ? (
            <>
              ...
              {actor && actor.uid !== snap.uid ? (
                <ToolbarButton label="View as" icon="eye-outline" onPress={handleImpersonate} />
              ) : null}
            ...
            {aiSummary ? (
              <View style={styles.aiCard}>
                <View style={styles.aiCardHead}>
                  ...
                  <Text style={styles.aiCardLabel}>AI summary · 30-day activity</Text>
                </View>
                <Text style={styles.aiCardBody}>{aiSummary}</Text>
                <Text style={styles.aiCardFoot}>Generated by Claude — review before relying on it.</Text>
              </View>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  aiCardHead: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  aiCardLabel: { fontSize: 11, fontWeight: '700', color: Colors.primary, letterSpacing: 1.2, textTransform: 'uppercase' },
  aiCardBody: { fontSize: FontSize.sm, color: Colors.textDark, lineHeight: 21 },
  aiCardFoot: { fontSize: 10, color: Colors.textMuted, marginTop: 4, fontStyle: 'italic' },
});
